...
export const parseAiAssistantResponse = (payload: unknown): AiAssistantResponse => {
  ...
  switch (payload.action) {
    ...
    case 'convert_to_tasks':
      if (!Array.isArray(payload.tasks) || !payload.tasks.every((task) => typeof task === 'string'))
        throw new Error('Invalid convert_to_tasks response')
    ...
    case 'classify_note':
      if (
        ...
        throw new Error('Invalid classify_note response')
    ...
    case 'edit_note': {
      ...
      if (hasCategory && !categories.includes(payload.category as NoteCategory))
