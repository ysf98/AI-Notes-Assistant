...
export const parseAiAssistantResponse = (payload: unknown): AiAssistantResponse => {
  ...
  switch (payload.action) {
    case 'create_note':
      if (typeof payload.title !== 'string' || typeof payload.content !== 'string' || typeof payload.category !== 'string' || !categories.includes(payload.category as NoteCategory)) {
        throw new Error('Invalid create_note response')
      }
    ...
    case 'propose_title':
      if (typeof payload.title !== 'string') throw new Error('Invalid propose_title response')
      return { action: 'propose_title', title: payload.title }
    case 'classify_note':
