...
export const ChatPanel = ({ messages, value, isLoading, onChange, onSend }: ChatPanelProps) => (
  <section className="w-full xl:w-96 border-l border-slate-200 dark:border-slate-800 bg-slate-300 dark:bg-slate-950 p-4 transition-all duration-300">
    <div className="h-full rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 backdrop-blur-sm p-4 flex flex-col">
      <div className="mb-3">
      ...
      <div className="flex-1 space-y-2 overflow-y-auto pr-1">
        {messages.length === 0 ? (
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/70 p-4 text-sm text-slate-600 dark:text-slate-300">
            <p className="font-medium text-slate-900 dark:text-slate-100">Todavía no hay conversación</p>
