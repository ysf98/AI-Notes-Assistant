    ...
    <div className="h-full rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 backdrop-blur-sm p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={onCreate}
          className="rounded-lg bg-slate-800 px-3 py-2 text-sm font-medium text-slate-100 hover:bg-slate-700 dark:bg-slate-200 dark:text-slate-900 dark:hover:bg-slate-300 transition-all duration-200 active:scale-[0.99]"
        >
          + Nueva
        </button>
      </div>
      ...
      <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1">
        ...
        {!isLoading && notes.length === 0 && !query ? (
          <div className="card-ui p-4 text-sm text-slate-600 dark:text-slate-300">
            <p className="font-medium text-slate-900 dark:text-slate-100">No hay notas todavía</p>
            <p className="mt-1">Crea tu primera nota para empezar.</p>
          </div>
        ) : null}
        {!isLoading && notes.length === 0 && query ? (
          <div className="card-ui p-4 text-sm text-slate-600 dark:text-slate-300">
